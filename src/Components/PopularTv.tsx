...
const PopularTv = () => {
  ...
  const tvMatch = useMatch(`/tv/popular/:tvId`);
  const tvNavigate = useNavigate();
  const { data: popularTvData } = useQuery(["tv", "popular"], GetPopularTvs);
  ...
  const goMovieDetail = (tvId: number) => tvNavigate(`/tv/popular/${tvId}`);
  ...
  return (
    <>
      <SliderTitle onClick={incraseIndex}>Popular Tv Shows</SliderTitle>
      <Slider>
        ...
        >
          ...
          >
            {popularTvData?.results
              ...
              .map((tv: ITv) => (
                <PosterCard
                  layoutId={tv.id.toString() + "popular"}
        ...
        {tvMatch ? (
          <>
            ...
            <Detail
              ...
              layoutId={tvMatch?.params.tvId + "popular"}
            >
              <DetailImg
                src={
                  detailTv
